...
```
All the output will be saved under folder `KuaiRand-27K-with_feature/output_{dataset}`. dataset can be `["KuaiRand-27K", "KuaiRand-27K-0501","KuaiRand-27K-100krows"]`

1. Data split by timesamp
```sh
python 1_1_generate_positive_train_test_based_on_timestamp.py --dataset KuaiRand-27K 
```
- If you use the full data, just run like this.
- If you use part data, do change the --dataset 
...
bash run.sh
```
- If you use the full data, just bash run.sh
- If you use part data, do change the --dataset in run.sh
- You can change the parameters in run.sh to finetune them

You will get `best_item_embeddings.npy` in `f"output_{args.dataset}` 从第0行开始，第k行就是对应id=k的item.

(运行位于`SASRec.pytorch/python`文件夹中的`run.sh`就行，可能唯一要改的就是`--feature_path`和`--input_csv_path`的文件地址，同样地读取train文件和特征文件，输出的是checkpoint。)
