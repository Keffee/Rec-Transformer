...
def main():
    ...
    # =========================================================================
    # --- 关键修改 1: 调整 `prompt` 的 Schema 定义 ---
    # 从 "Sequence of Structs" 改为 "Struct of Sequences"
    # =========================================================================
    final_features = datasets.Features({
        'data_source': datasets.Value('string'),
        'ability': datasets.Value('string'),
        'prompt': {
            # datasets.Sequence(...) 等价于 [datasets.Value(...)]
        ...
        'extra_info': {
            ...
            'original_index': datasets.Value('int64'),
    ...
    def process_dataframe_to_list(df, split_name):
        ...
        for idx, row in df.iterrows():
            ...
            processed_data = {
                ...
                # =============================================================
                # --- 关键修改 2: 调整 `prompt` 的数据结构 ---
                # 从 [{"key": "value"}] 改为 {"key": ["value"]}
                # =============================================================
                "prompt": {
    ...
    processed_train_dataset = datasets.Dataset.from_list(train_data_list, features=final_features)
    
    if test_data_list:
        processed_test_dataset = datasets.Dataset.from_list(test_data_list, features=final_features)
    else:
