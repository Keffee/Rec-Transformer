...
def main():
    ...
    parser.add_argument("--user_id_csv", type=str, default='1_positive_data_100k.csv', help="包含 user_id 的CSV文件路径，需与JSON文件行对应。")
    # ------------------------------------
    parser.add_argument("--output_dir", type=str, default='./6_parquet_for_verl', help="输出Parquet文件的目录。")
    parser.add_argument("--data_source_name", type=str, default="KuaiRand-27K-100krows", help="为数据源指定一个名称。")
    ...
    def make_map_fn(split_name):
        def process_fn(example, idx):
            ...
            if len(tokens) >= 3:
                ...
                ground_truth_tokens = tokens[-3:]
                
                prompt_content = " ".join(prompt_tokens)
                ground_truth = " ".join(ground_truth_tokens)
            else:
                # 处理序列长度小于3的边界情况
                print(f"警告：在处理索引 {idx} 时，token 数量 ({len(tokens)}) 小于3。将使用全部内容作为 ground_truth。")
                prompt_content = ""
